...
public class PackageDeliveredParsingStrategy implements EventPayloadHandler<PackageDelivered> {
    ...
    public EventTrackingType getTypeOfEvent() {
        return EventTrackingType.PACKAGE_DELIVERED;
    }

    @Override
    public void handle(PackageDelivered payload) throws JsonProcessingException {
        ...
        packageManagementService.createOrUpdatePackag(payload.getPackageId(), payload.getOrderId(),getTypeOfEvent(),aPackage -> {
            packageMapper.updateFromPackageDelivered(payload,aPackage);

        });
    }

    @Override
    public Class<PackageDelivered> getPayloadType() {
        return PackageDelivered.class;
    }
